"""
FinSight AI — Enhanced Feature Engineering Pipeline (v2)

Architectural decisions
-----------------------
1.  **Separation of feature families** — each indicator group lives in its own
    method (``_price_features``, ``_momentum_features``, ``_regime_features``,
    etc.).  This makes individual groups testable, switchable, and extendable
    without touching unrelated code.

2.  **Feature selection as a first-class pipeline stage** — correlation
    filtering, variance thresholding, and mutual-information selection are
    composed in ``FeatureSelector`` rather than scattered through the builder.
    This keeps the ``FeatureEngineer`` responsible only for computation, not
    selection.

3.  **No forward-looking bias** — every rolling/EWM operation uses only
    data available at the current bar.  Lag-1 shift is applied before any
    cross-sectional normalisation.

4.  **Multi-horizon label generation** — target labels for 1d, 5d, 21d, and
    126d horizons are computed once and stored as separate columns.  Each
    model can pick the label it needs without a separate data pass.

5.  **Regime detection as features** — volatility and trend regimes are
    estimated from the same rolling windows used elsewhere so they are
    fully reproducible.

Advanced signals added vs v1
-----------------------------
- Market-regime detection (vol regime, trend regime)
- Volatility regime (realized vol percentile)
- Cross-sectional momentum persistence
- Volume imbalance (buy-pressure proxy)
- Price microstructure (Amihud illiquidity, spread proxy)
...
def compute_garman_klass_vol(
    ...
    return gk.rolling(period, min_periods=period).mean().apply(
        lambda x: np.sqrt(max(x, 0) * 252)
    )
...
def compute_hurst(series: pd.Series, lags: int = 20) -> pd.Series:
    ...
    Computationally expensive — computed on log-returns with a rolling
    window of ``lags * 2`` bars.
    """
...
def compute_vol_regime(
    realized_vol: pd.Series, lookback: int = 252
) -> pd.Series:
    """
    ...
    This is a market-condition signal rather than a raw indicator.
    """
    return realized_vol.rolling(lookback, min_periods=60).rank(pct=True)


def compute_trend_regime(close: pd.Series, fast: int = 50, slow: int = 200) -> pd.Series:
    """
    Simple trend-regime indicator: +1 (uptrend), -1 (downtrend), 0 (neutral).

    Uses the relationship between two SMAs.  Fast > slow → uptrend.
    """
...
def compute_volume_imbalance(
    close: pd.Series, open_: pd.Series, volume: pd.Series, period: int = 20
) -> pd.Series:
    """
    Volume imbalance — buy pressure proxy.

    Positive close relative to open suggests buyer-initiated volume.
    Negative → seller-initiated.  Rolling mean smooths the signal.
    """
...
class FeatureSelector:
    ...
    Design rationale
    ----------------
    Running all three stages avoids the pitfalls of using any single method:
    - Variance thresholding alone misses correlated-but-variable redundancies.
    - Correlation filtering alone can discard informative pairs.
    - MI alone is expensive on large feature sets; the earlier stages reduce cost.

    The selector is *fit on training data only* and applied to test/inference
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series) -> "FeatureSelector":
        ...
        logger.info("After correlation filter (%.2f): %d features", self.correlation_threshold, len(cols))
...
class FeatureEngineer:
    ...
    Builds a comprehensive feature matrix from raw OHLCV data with:
    - classic technical indicators (RSI, MACD, BB, ATR, OBV, VWAP)
    - advanced quantitative signals (Garman-Klass vol, Hurst, Amihud)
    - regime features (vol regime, trend regime)
    - microstructure signals (volume imbalance, illiquidity)
    - higher-order moments (rolling skew, kurtosis)
    ...
    compute_hurst_exp: bool
        Whether to compute the Hurst exponent (expensive, ~2s on 1000 rows).
    """
    ...
    def get_feature_columns(self, df: pd.DataFrame) -> list[str]:
        ...
        target_cols = set(settings.TARGET_COLUMN)
        horizon_targets = {f"target_{h}" for h in HORIZONS}
        exclude = {"Open", "High", "Low", "Close", "Volume"} | horizon_targets | {settings.TARGET_COLUMN}
    ...
    def split_X_y(
        ...
        if target_col not in df.columns:
            # Fallback to default target for backward compatibility
            target_col = settings.TARGET_COLUMN

        feature_cols = self.get_feature_columns(df)
        # Drop any horizon targets that leaked into features
    ...
    def _volatility_features(self, feat: pd.DataFrame) -> pd.DataFrame:
        ...
        # Hurst exponent (optional — expensive)
    ...
    def _moving_average_features(self, feat: pd.DataFrame) -> pd.DataFrame:
        ...
        # EMA cross (golden/death cross proxy)
    ...
    def _bollinger_features(self, feat: pd.DataFrame) -> pd.DataFrame:
        ...
        feat["bb_squeeze"] = feat["bb_width"] < feat["bb_width"].rolling(20).quantile(0.2)
        feat["bb_squeeze"] = feat["bb_squeeze"].astype(int)
        return feat

    def _regime_features(self, feat: pd.DataFrame) -> pd.DataFrame:
        close = feat["Close"]
        # Volatility regime
        rv20 = feat.get("realized_vol_21d", feat["log_returns"].rolling(21).std() * np.sqrt(252))
    ...
    def _candlestick_features(self, feat: pd.DataFrame) -> pd.DataFrame:
        ...
        feat["candle_dir"]     = (close > open_).astype(int)   # 1 = bullish bar
        return feat
    ...
    def _target_labels(self, feat: pd.DataFrame) -> pd.DataFrame:
        ...
        # Default target (1d) for backward compatibility
