"""
FinSight AI — News Recency Validation
======================================

Enforces strict lookback windows for news articles retrieved from DDG
(DuckDuckGo) or any other source that does not guarantee result freshness.

Problem
-------
DDG ``text()`` results can surface articles that are weeks, months, or even
years old — the rank signal is relevance, not recency.  A "NVDA earnings beat"
headline from 18 months ago is factually accurate but operationally misleading:
the signal fusion pipeline would weight it as *current* bullish evidence for a
ticker whose recent results may be entirely different.

Solution architecture
---------------------
This module provides:

1. ``ArticleDateExtractor`` — attempts to extract a publish date from:
   a. DDG result metadata fields (``published``, ``date``).
   b. The snippet text itself (regex for common date patterns).
   c. HTTP ``Last-Modified`` / ``og:article:published_time`` headers/meta
      (optional, only when a URL fetch is acceptable).

2. ``NewsRecencyFilter`` — applies a configurable ``max_age_days`` window.
   Articles whose date cannot be determined are handled via a configurable
   ``unknown_date_policy``:
   - ``"accept"`` — pass through with a staleness warning flag.
   - ``"reject"`` — discard; safest for live trading contexts.
   - ``"accept_with_penalty"`` — pass through but halve the ``final_weight``.

3. ``RecencyAuditEntry`` — lightweight dataclass attached to each ``NewsItem``
   after filtering so callers can see *why* an article was kept or dropped.

Design decisions
----------------
* No mandatory HTTP re-fetch for date extraction.  A HEAD request per article
  would add 8–12 network round-trips to every prediction call.  The extractor
  is best-effort: regex on snippet first, metadata fields second.
* ``max_age_days`` is configurable via ``settings`` with a sensible default
  of 7 days for intraday / next-day predictions.  Longer horizons (1 m, 6 m)
  tolerate older news.
* The filter is designed to be inserted in ``FinancialIntelligenceService``
  between retrieval and analysis — zero changes to ``NewsRetriever``.

Lookback window policy by horizon
----------------------------------
+---------+---------------------+
| horizon | max_age_days        |
+---------+---------------------+
| 1d      | 3  (72 h)           |
| 7d      | 7  (1 week)         |
| 1m      | 30 (1 month)        |
